...
def paths_for_localized_resources(root, target_name):
    return [os.path.abspath(path) for path in glob.glob(root + "/" + target_name + "/*.lproj/*.strings")]
...
def add_file_reference(project, path, variant_group):
    file_reference = PBXFileReference.Create(path, name=locale_name_from_path(path))
    ...
    for target_name in TARGETS:
        ...
        if target and parent_group and phase:
            ...
            for path in paths_for_localized_resources(sys.argv[2], target_name):
                ...
                file_reference = add_file_reference(project, path, variant_group)
